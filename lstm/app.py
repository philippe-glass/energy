...
def train_model_incremental(model_key, model_df, model_df_last):
    if not data_store.has_model(model_key):
        # init a new model
        X_depth, Y_depth = 10, 10
        input_shape = (X_depth, 1)
        load_existing_weights = False
        new_regressor, sampling_nb, sc = data_store.init_model_LSTM(input_shape, Y_depth, model_key,
    ...
    train_depth = 60
    train_depth = 10
    X_both, y_both = None, None
    # sc = MinMaxScaler(feature_range=(0, 1))
    test_rate = 0.2
    #X_Train_old, X_test_old, Y_Train_old, Y_test_old, indexes_train_old, indexes_test_old = prepare_data(model_df, train_depth, test_rate, sc)
    X_Train_old, X_Train_new, Y_Train_old, Y_Train_new, indexes_train_old, indexes_train_new = prepare_data(model_df, train_depth, 0.05, sc)
    #X_Train_old, X_Train_new, Y_Train_old, Y_Train_new, indexes_train_old, indexes_train_new = prepare_data(model_df, train_depth, 0.01, sc)

    if debug_level > 0:
        print("X_Train_old.shape", X_Train_old.shape)
        print("X_Train_new.shape", X_Train_new.shape)
    X_Train_old = np.reshape(X_Train_old, newshape=(X_Train_old.shape[0], X_Train_old.shape[1], 1))
    X_Train_new = np.reshape(X_Train_new, newshape=(X_Train_new.shape[0], X_Train_new.shape[1], 1))
    if debug_level >= 0:
        logger.info("train_model " + model_key.generate_key() + " X_Train_old.shape = " + str(X_Train_old.shape)
                     + " X_Train_new.shape = " + str(X_Train_new.shape))
    #Y_Train = np.reshape(Y_Train, newshape=(Y_Train.shape[0]))
    Y_Train_old = np.reshape(Y_Train_old, newshape=(Y_Train_old.shape[0]))
    Y_Train_new = np.reshape(Y_Train_new, newshape=(Y_Train_new.shape[0]))
    ...
    if True or variable == "consumed":
        ...
        #X_both, y_both = vstack((X_Train_old, X_Train_new)), hstack((Y_Train_old, Y_Train_new))
        # fit the model on new data
        #regressor.fit(X_both, y_both, epochs=data_store.nb_epochs, batch_size=32, verbose=0)
        regressor.train_on_batch(X_Train_new, Y_Train_new)


    t_after = time.time()
    time_elapsed_sec = t_after - t_before
    sampling_nb = len(model_df.index)
    new_sampling_nb = len(indexes_train_new)
    logging.info("After incremental model train " + model_key.generate_key() + " : time elapsed (sec) = " + str(time_elapsed_sec)
                 + ", used samples = " + str(new_sampling_nb) + ", total samples = " + str(sampling_nb))
    #sampling_nb = sampling_nb + X_Train_new.shape[0]
    data_store.put_model(regressor, sampling_nb, model_key, sc)
    data_store.dump_model(model_key)
    model_info = data_store.get_model_info2(model_key)
    return model_info
...
def aux_load_node_history(json_request, is_simulation):
    ...
    #is_simulation = json_request.get('isSimulation')
    ...
    df_node_history = loadNodeHistory(scope, node_name, date_min, date_max, is_simulation)
...
def initNodeHistory():
    ...
    node_name = json_request.get('nodeName')
    result = []
    df_node_history, date_min, date_max = aux_load_node_history(json_request, True)
    ...
    try:
        for variable in list_variables:
            ...
            # result.append(1+model_info) Just to test exception logging
...
def add_node_history():
    ...
    node_name = json_request.get('nodeName')
    result = []
    df_node_history, date_min, date_max = aux_load_node_history(json_request, None)
    # df = prepare_df(request.json, list_variables)
    ...
    try:
        for variable in list_variables:
            ...
            if use_incremental_learning:
                #df_node_history_last, date_min, date_max = aux_load_node_history(json_request, False)
                #df2_last = df_node_history_last[[variable]].resample('1min').mean()
                #nan_count_last = check_model_df(df2, variable)
                #if nan_count_last > 0:
                #    logger.info("add_node_history : nan_count_last = " + str(nan_count_last) + " for key " + model_key.generate_key())
                #    df2_last = correct_model_df(df2_last, variable)
                #df3_last  = df2_last.dropna(subset=[variable])
                model_info = train_model_incremental(model_key, df3, None)
            else:
...
def prediction1():
    ...
    logging.info("prediction1 : " + model_key.generate_key()+ " step1")
    ...
    try:
        ...
        print("Exception", err)
        traceback.print_exc(file=sys.stdout)
        logging.error("prediction1 : " + model_key.generate_key() + " Thrown exception:" + str(err))
    logging.info("prediction1 : " + model_key.generate_key() + " step2" + " before regressor.predict")
    ...
    try:
        ...
        traceback.print_exc(file=sys.stdout)
        logging.error("prediction1 : " + model_key.generate_key() + " Thrown exception:" + str(err))
    logging.info("prediction1 : " + model_key.generate_key() + " after regressor.predict")
    ...
    result = {"listDatesX": list_dates_X, "listY": list_true, "listPredicted": Y_predicted_list,
              "current_time": current_time, "horizons": horizons, "predictionDates": prediction_str_dates}
    t_after = time.time()
    time_elapsed_sec = t_after - t_before
    logging.info("prediction1 : " + model_key.generate_key() + " end " + " : time elapsed (sec) = " + str(time_elapsed_sec))
    return jsonify(result)
