...
-- 	PRIMARY KEY (`id`),
	UNIQUE (id_time_window, variable_name, location, scenario),
...
;
DROP TABLE IF EXISTS `transition_matrix_cell`
;
CREATE TABLE `transition_matrix_cell` (
	...
	`obs_number`					INTEGER  NOT NULL DEFAULT 0.0,
	`corrections_number`			INTEGER  NOT NULL DEFAULT 0.0 , -- Used for self-adaptive correction
	`value`							DECIMAL(10,5) NOT NULL DEFAULT 0.0,
	`rowsum`						INTEGER NOT NULL DEFAULT 0.0,
	FOREIGN KEY(id_transition_matrix) REFERENCES transition_matrix(id),
	UNIQUE (id_transition_matrix, row_idx,  column_idx)
)
...
CREATE TABLE `transition_matrix_iteration` (
	...
	`last_update`				DATETIME NOT NULL DEFAULT '1970-01-01 00:00:00',
-- 	PRIMARY KEY (`id`),
	UNIQUE (id_transition_matrix, end_date),
	FOREIGN KEY(id_transition_matrix) REFERENCES transition_matrix(id)
)
...
CREATE TABLE `transition_matrix_cell_iteration` (
	`creation_time` 					DATETIME NOT NULL DEFAULT (datetime(CURRENT_TIMESTAMP, 'localtime')),
	`id_transition_matrix_iteration` 	INTEGER  NOT NULL,
    `id_transition_matrix`	 			INTEGER  NOT NULL,
	`row_idx`							TINYINT  NOT NULL,
	`column_idx`						TINYINT  NOT NULL,
	`obs_number`						INTEGER  NOT NULL DEFAULT 0.0,
	`corrections_number`				INTEGER  NOT NULL DEFAULT 0.0 , -- Used for self-adaptive correction
	FOREIGN KEY(id_transition_matrix_iteration) REFERENCES transition_matrix_iteration(id),
	FOREIGN KEY(id_transition_matrix) REFERENCES transition_matrix(id),
	UNIQUE (id_transition_matrix_iteration, row_idx,  column_idx)
)
...
-- 	PRIMARY KEY (`id`),
	...
	FOREIGN KEY(id_correction) REFERENCES log_self_correction(id)
)


;
CREATE INDEX _id_target_state_histo ON `prediction` (id_target_state_histo);
CREATE INDEX _creation_date ON `prediction` (creation_date);
CREATE INDEX _creation_day ON `prediction` (creation_day);
CREATE INDEX _initial_date ON `prediction` (initial_date);
CREATE INDEX _target_date ON `prediction` (target_date);
CREATE INDEX _target_date_varname ON `prediction` (target_date, variable_name);
CREATE INDEX _target_day_hour_var2 ON `prediction` (target_day, target_hour, variable_name, horizon_minutes , use_corrections);
...
-- 	 PRIMARY KEY (`id`),
	UNIQUE (id_context, date, variable_name),
...
;
CREATE INDEX _date ON `state_history` (date);
DROP TABLE IF EXISTS `log_self_correction`
;
