...
public class PredictionDeviation implements Serializable {
	...
	private TransitionMatrixKey transitionMatrixKey;
	private MarkovState initialState;
	private MarkovState stateUnder;
	private MarkovState stateOver;
	...
	public PredictionDeviation(TransitionMatrixKey _transitionMatrixKey
			,MarkovState initialState
			,MarkovState _stateUnder
			,MarkovState _stateOver
			,Double _excess
	...
	public MarkovState getStateUnder() {
		return stateUnder;
	}

	public void setStateUnder(MarkovState actualState) {
		this.stateUnder = actualState;
	}

	public MarkovState getStateOver() {
		return stateOver;
	}

	public void setStateOver(MarkovState _predictedState) {
		this.stateOver = _predictedState;
	}

	public TransitionMatrixKey getTransitionMatrixKey() {
		return transitionMatrixKey;
	}

	public void setTransitionMatrixKey(TransitionMatrixKey transitionMatrixKey) {
		this.transitionMatrixKey = transitionMatrixKey;
	}

	public int getStartHour() {
		return transitionMatrixKey.getTimeWindow().getStartHour();
	}
	...
	public static String formatKey(TransitionMatrixKey _usedTransitionMatrixKey, MarkovState _actualState,
			MarkovState _predictedState) {
	...
	public String toString() {
		StringBuffer result = new StringBuffer();
		result.append("")
				.append(transitionMatrixKey.getVariable() + " at " + transitionMatrixKey.getTimeWindow().getStartHour())
		...
		for (Date nextTargetDate : listTargetDates) {
			...
			if (idx < listVectorDifferential.size()) {
				...
				//}
				result.append("[").append(UtilDates.df2.format(differential)).append("]");
			}
			idx++;
		}
		return result.toString();
	...
	public static List<PredictionDeviation> aggregateDeviations(List<PredictionDeviation> listSingleDeviations,
		...
		for (PredictionDeviation singleDeviaiton : listSingleDeviations) {
			TransitionMatrixKey trMatrixKey = singleDeviaiton.getTransitionMatrixKey();
			...
			MarkovState actualState = singleDeviaiton.getStateUnder();
			MarkovState predictedState = singleDeviaiton.getStateOver();
			//MarkovState initialState = singleDeviaiton.getInitialState();
